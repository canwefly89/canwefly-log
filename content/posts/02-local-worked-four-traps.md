...
> 🖼️ *[짤 자리: "Works on my machine" 클래식 밈]*

<br>

## 에피소드 1 — `async def`를 썼는데 서버가 통째로 멈췄다 (Sprint 2, RAG)
...
겉보기엔 요즘 파이썬 스타일이다. 근데 이게 **정확히 최악의 조합**이었다.
...
| `async` 안의 블로킹 호출 | 애초에 드물다 | **아주 흔하다** (많은 SDK가 동기) |
...
파이썬 C/C++ 바인딩 패키지의 오래된 특성이다. JS의 npm 패키지처럼 "한 번 깔면 모든 환경에서 똑같이 동작"이라는 감각으로 접근하면 어디선가 이런 문제를 만나게 된다. 이번 케이스는 에러 메시지도 조용한 편이었기에 한참 헤맸다.
...
## 에피소드 3 — 병렬화 코드는 그대로였는데, 효과가 14배 달랐다 (Sprint 3, 멀티모달)
...
같은 병렬화 코드인데 **로컬에서는 거의 효과가 없었고, 배포에서는 극적으로 줄었다.**

> 🖼️ *[짤 자리: "왜 똑같은 일을 했는데 결과가 다르지?" 어리둥절한 밈]*
...
25장 프레임을 순차 호출하느냐 3개씩 동시 호출하느냐의 차이가 압도적이었다. 파이프라인 단위의 병렬화보다 **작업 단위의 동시 호출**이 훨씬 효과적이다.
...
## 에피소드 4 — "OpenAI 호환"이라더니, 속 타입이 달랐다 (Sprint 3, 멀티모달)
...
런타임·인프라 레이어의 이야기는 여기까지. 다음 편은 한 층 위 — **"AI 모델과 평가 체계가 품고 있는 반직관"**을 다룬다.

- **3편** — [지표는 올랐는데 체감은 나빠졌다 — RAG 평가의 역설](/posts/rag-eval-five-paradoxes)
  - Vision 해상도를 올렸더니 환각이 늘었다
  - 임베딩 모델마다 유사도 분포가 이렇게 다를 줄이야
  - "올바른 거부"가 0점 받는 평가 함수의 역설
  - 검색은 맞았는데 답변은 틀렸다
  - 같은 설정 2번 돌렸더니 0.80 vs 0.40

이 에피소드들은 인프라가 아니라 **"측정을 똑바로 한다는 것의 어려움"**에 대한 이야기다.
...
- 이 글의 내용은 Sprint 2 (RAG) 와 Sprint 3 (멀티모달) 과제 전반에서 실제로 마주친 문제들을 정리한 것
