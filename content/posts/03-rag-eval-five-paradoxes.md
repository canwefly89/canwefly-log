...
> 🖼️ *[짤 자리: "숫자는 올랐는데 왜 기분은 나빠지지" 혼란 밈]*
...
업로드 한 번에 Vision API 50건 호출, 총 **142초 이상**. 사용자가 PDF 업로드 후 2분 넘게 기다리는 건 말이 안 된다. 비용도 부담.
...
그런데 모델을 바꿔가며 실험해보니 이 숫자가 **모델마다 완전히 다른 의미**를 갖는다는 걸 알게 됐다.
...
반대로 `nomic-embed-text`는 모든 한국어 텍스트를 **0.65~0.87의 좁은 범위**에 몰아넣었다. "이 식당의 주소는?" 같은 완전히 무관한 질문도 유사도 0.88로 나온다. 이 데이터에서는 threshold 필터링이 **사실상 무의미**했다.

> 🖼️ *[짤 자리: "같은 점수인데 의미가 완전 달라?" 혼란 밈]*
...
대부분의 RAG 튜토리얼은 **영어 데이터**로 작성된다. 영어에서는 이 세 모델의 유사도 분포가 훨씬 더 일관되게 나온다. 한국어에서만 이렇게 극단적인 차이가 나타난다.
...
이 연재의 마지막 편은 앞의 세 편을 정리한 **실무 구축 플레이북**이다. "회사에서 RAG 붙여달라는 요청이 왔을 때, 내가 어떤 순서로 어떤 의사결정을 할 것인가"를 4-Phase로 정리했다.

- **4편** — [회사에서 "RAG 붙여주세요" 했을 때 — 내가 쓰는 4-Phase 구축 플레이북](/posts/rag-playbook-four-phases)

특히 이번 편에서 본 **"평가 체계의 중요성"** 이 4편의 Phase 2 전체를 차지한다. "평가 체계 없이 고도화로 넘어가면 전부 미신"이라는 교훈이 어디서 왔는지, 본인이 직접 겪어봐야 진짜 박히는 감각이다.
...
- 이 글의 내용은 Sprint 2 (RAG) + Sprint 3 (멀티모달) + Sprint 4 (고도화) 과정에서 반복적으로 마주친 평가·모델 레이어의 구조적 문제들을 정리한 것
